...
void destroy_remaining_heap_allocations(Matrix_t **mats, unsigned int num_mats);
	...
	/*
	* Purpose: run the bulk of the program by utilizing function calls
	* Input:
	* Return: 0 if the program finished running successfully.
	*/
int main (int argc, char **argv) {
	...
	if(add_matrix_to_array(mats,temp, 10) == UINT_MAX){
		perror("No matrix was added to the array\n");
		return -1;
	...
	if(!(write_matrix("temp_mat", mats[mat_idx]))){
		perror("Failed to write the matrix\n");
		return -1;
	} // TODO ERROR CHECK
	...
	//TODO FUNCTION COMMENT
	/*
	* Purpose: execute the given commands that will manipulate the
	* matrices in the matrix table.
	* Input: table of commands given by the user, the matrix table
	* and the number of matrices in the table.
	* Returns: nothing.
	*/
void run_commands (Commands_t* cmd, Matrix_t** mats, unsigned int num_mats) {
	//TODO ERROR CHECK INCOMING PARAMETERS
	if (!(cmd)){
		perror("No commands recieved\n");
		return;
	}
	if (!(*mats)){
		perror("Matrix table not found\n");
		return;
	}
	if (num_mats == NULL){
		perror("number of matrices not recieved\n");
		return:
	}
	...
	if (strncmp(cmd->cmds[0],"display",strlen("display") + 1) == 0
		...
		&& cmd->num_cmds == 4) {
			...
			if (mat1_idx >= 0 && mat2_idx >= 0) {
				...
				if(add_matrix_to_array(mats,c, num_mats) == UINT_MAX){
		...
		if (mat1_idx >= 0 ) {
				...
				if(!(duplicate_matrix (mats[mat1_idx], dup_mat))){
					perror("Failed to duplicate matrix\n");
					return;
				} //TODO ERROR CHECK NEEDED
				if(add_matrix_to_array(mats,dup_mat,num_mats) == UINT_MAX){
					perror("Failed to add matrix to array\n");
					return;
				} //TODO ERROR CHECK NEEDED
		...
		if(add_matrix_to_array(mats,new_matrix, num_mats) == UINT_MAX){
			perror("Failed to add matrix to the array\n");
			return;
		} //TODO ERROR CHECK NEEDED
		...
		if(add_matrix_to_array(mats,new_mat,num_mats == UINT_MAX){
			perror("Failed to add matrix to the array\n");
			return;
		} // TODO ERROR CHECK NEEDED
		...
		if(!(random_matrix(mats[mat1_idx],start_range, end_range))){
			perror("Failed to create random matrix\n");
			return;
		} //TODO ERROR CHECK NEEDED
	...
	//TODO FUNCTION COMMENT
	// searches through the list of matrixs for a specific one.
	// need by the program.
	// returns the index value of the desired matrix in the list.
unsigned int find_matrix_given_name (Matrix_t** mats, unsigned int num_mats, const char* target) {
	//TODO ERROR CHECK INCOMING PARAMETERS
	if (!(*mats)){
		perror("Matrix table not provided\n");
		return -1;
	}
	if (num_mats == NULL){
		perror("number of matrices not recieved\n");
	...
	if (!(target)){
		perror("no target matrix recieved\n");
		return -1;
	}
	...
	//TODO FUNCTION COMMENT
	/*
	* Purpose: free up the memory still allocated to elements of 
	* the program.
	* Input: the matrix table and the number of matrices in the table.
	* Returns: no value
	*/
void destroy_remaining_heap_allocations(Matrix_t **mats, unsigned int num_mats) {
	...
	if(num_mats == NULL){
		perror("No valid value for num_mats recieved");
		return;
	}
	// COMPLETE MISSING MEMORY CLEARING HERE
	for (int i=0; i<(*mats)->num_mats; ++i){
		free((*mats)->mats[i]);
	}
	free((*mats)->mats);
	free((*mats));
